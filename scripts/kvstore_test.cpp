...
const int buffer_entity2id_build = 8;
const long long buffer_size = Util::MAX_BUFFER_SIZE * buffer_entity2id_build;
const string storepath = "./test";
const string filename = "entity2id.dat";
const string data = "data1";
const int maxn=10000000;

KVstore testkv(storepath);
...
void test_subID2values_get_s2o()
{
	...
	for(int i=0;i<n;i++)
	{
		...
		unsigned *olist;
		unsigned len;
		testkv.getobjIDlistBysubID(s,olist,len, false);
		delete[] olist;
	}
	gettimeofday(&end_time,NULL);
	cout<< "time(s) : " << end_time.tv_sec-start_time.tv_sec + (end_time.tv_usec-start_time.tv_usec)/1000000.0 <<endl;
